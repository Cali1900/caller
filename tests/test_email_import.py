...
def test_an_imported_lead_with_a_phone_added_by_hand_dials(db, cfg_env, monkeypatch):
    ...
    with db.cursor() as cur:
        cur.execute("""UPDATE leads SET phone_e164 = '+14245557788',
...
def test_the_contact_save_refuses_a_phone_without_a_timezone(db, client):
    ...
    assert 'REJECTED' in r.headers['location'], \
        'a phone was saved with no timezone - the lead would look dialable and ' \
        'be permanently filtered out'
...
def test_step_1_is_due_immediately_for_a_lead_with_no_emailed_at(db, dripc):
    ...
    assert lead['drip_campaign_id'] is not None, 'the batch was not routed'
...
def test_email_1_counts_as_step_1_for_a_call_sourced_lead(db, dripc):
    ...
    campaigns.update(running_campaign_id(),
                     default_drip_id=dripc['campaign_id'])
    assert stages.mark_emailed(lid, emailed_by='operator') is not None

    after = _get(db, lid)
    assert after['drip_campaign_id'] == dripc['campaign_id'], \
        'default_drip_id did not route the lead'
...
def test_the_call_campaign_chooses_which_drip_its_leads_enter(db, dripc):
    """
    ⚠️ WITHOUT THIS, A SECOND DRIP SILENTLY STOPS THE FIRST'S FOLLOW-UPS.

    only_drip() assigns only when exactly ONE drip runs. With two running - for
    any reason - every call-sourced lead that got email 1 joined NO drip: email
    out, lead at 'emailed', nothing following up, and nothing saying so.
    """
    other = campaigns.create('OTHER-DRIP', campaign_type='drip')['campaign_id']
    campaigns.start(other)
    assert drip.only_drip() is None, 'the premise: two drips run, so no fallback'

    campaigns.update(running_campaign_id(),
                     default_drip_id=dripc['campaign_id'])
    src = campaigns.get(running_campaign_id())
    got = drip.drip_for(src)
    assert got is not None and got['campaign_id'] == dripc['campaign_id'], \
        'the campaign named a drip and was not routed to it'


def test_a_default_pointing_at_a_stopped_drip_routes_nowhere(db, dripc):
    """It must NOT fall back to whatever else is running - that is how a lead
    lands on the wrong copy."""
    other = campaigns.create('OTHER-DRIP2', campaign_type='drip')['campaign_id']
    campaigns.start(other)
    campaigns.update(running_campaign_id(),
                     default_drip_id=dripc['campaign_id'])
    campaigns.stop(dripc['campaign_id'])
    src = campaigns.get(running_campaign_id())
    assert drip.drip_for(src) is None


def test_only_drip_is_still_the_fallback_for_one_drip(db, dripc):
    """Naming one when there is only one would be ceremony."""
    src = campaigns.get(running_campaign_id())
    assert src['default_drip_id'] is None
    got = drip.drip_for(src)
    assert got is not None and got['campaign_id'] == dripc['campaign_id']

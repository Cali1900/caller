...
#
# NOTE this replaces only the DAYS branch. Step 1 has its own branch, timed in
# minutes from drip_entered_at, guarded by break 114.
TARGET = 'api/drip.py'
...
OLD = '''           "      AND l.emailed_at + (s.delay_days || ' days')::interval"
           "          <= now())"'''
NEW = '''           "      AND coalesce((SELECT max(es2.sent_at) FROM email_sends es2"
